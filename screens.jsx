...
const tabIcons = {
  ...
  trade:(a) => <g stroke={a?C.indigo:C.muted2} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none"><path d="M7 20V7"/><path d="M3.5 10.5 7 7l3.5 3.5"/><path d="M17 4v13"/><path d="M20.5 13.5 17 17l-3.5-3.5"/></g>,
...
const BottomTabs = ({ active, onNav }) => {
  ...
  ].filter(t => !(window.MMF_V1 && t.id === 'loan'));
  return (
    <div style={{
        position: 'absolute', left: 14, right: 14, bottom: 14, height: 64, zIndex: 20, borderRadius: 26,
          ...
          return (
            <div key={t.id} onClick={() => onNav && onNav(t.target)} style={{
              ...
              <div style={{
                ...
                <svg width="21" height="21" viewBox="0 0 24 24" fill="none">{tabIcons[t.id](a)}</svg>
              </div>
              <div style={{
                fontSize: 10, fontWeight: a ? 800 : 600,
  ...
  return (
    <Frame label={label}>
      ...
      <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
        ...
        {/* Loan section — omitted entirely in the V1 scope */}
        {loanState !== 'hidden' && <React.Fragment>
...
const TickerSheet = ({ item, onClose }) => {
  ...
  const segs = [
    ...
    { v: parts[1]||'', label:'Үр шим', c: C.indigo },
...
const Trading = ({ activeTab='trade', onNav }) => {
  ...
  return (
    <Frame label="24 — Trading / Marketplace">
      ...
      <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
        {window.AutoInvestEntry && React.createElement(window.AutoInvestEntry, { onNav })}
        {/* Анхдагч */}
...
const WHolding = ({ h, onNav }) => {
  ...
  return (
    <button onClick={()=>onNav && onNav('ownedDetail')} style={{
      width:'100%', textAlign:'left', display:'flex', alignItems:'flex-start', gap: 12, padding: 16,
      background:'#fff', border:`1px solid ${C.line2}`, borderRadius: 16, cursor:'pointer',
    }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        {/* company title + small logo */}
        <div style={{ display:'flex', alignItems:'center', gap: 8 }}>
          <div style={{ width: 24, height: 24, borderRadius: 7, background: h.c+'18', color: h.c, display:'flex', alignItems:'center', justifyContent:'center', fontWeight: 800, fontSize: 9.5, flexShrink: 0 }}>{h.ab}</div>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: C.ink, letterSpacing:'-0.01em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{h.issuer}</div>
          {window.aiPlanLive && window.aiPlanLive() && h.ticker === 'CAPIT 1450' && <span style={{ fontSize: 9.5, fontWeight: 800, color: C.indigo, background: C.indigoSoft, padding:'2px 7px', borderRadius: 999, flexShrink: 0 }}>АВТО</span>}
        </div>
        {/* ticker (smaller) + type */}
        <div style={{ fontSize: 11, color: C.muted, marginTop: 9, fontWeight: 600, fontVariantNumeric:'tabular-nums' }}>
          <span style={{ fontFamily:'JetBrains Mono, monospace', color: C.text, fontWeight: 600, letterSpacing:'0.02em' }}>{h.ticker}</span> · {h.type}
        </div>
        <div style={{ fontSize: 11, color: C.muted, marginTop: 3, fontWeight: 600 }}>Төлөгдөх {h.mat}</div>
        {badge && (
          <span style={{ display:'inline-flex', alignItems:'center', gap: 5, marginTop: 9, padding:'3px 8px', borderRadius: 999, background: badge.bg, color: badge.fg, fontSize: 10, fontWeight: 700 }}>
      ...
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ flexShrink:0, marginTop: 2 }}><path d="M9 6l6 6-6 6" stroke={C.muted2} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
    </button>
  );
};

const WCheckBox = ({ checked }) => (
  <span style={{ width: 22, height: 22, borderRadius: 7, flexShrink: 0, display:'flex', alignItems:'center', justifyContent:'center', background: checked ? C.indigo : '#fff', border:`2px solid ${checked ? C.indigo : C.line}`, transition:'all .12s' }}>
...
const WListing = ({ h, selectMode, checked, onToggle }) => (
  <div onClick={selectMode ? onToggle : undefined} style={{
    width:'100%', display:'flex', alignItems:'center', gap: 12, padding: 14,
    background:'#fff', border:`1.5px solid ${checked ? C.indigo : C.line2}`,
    borderRadius: 16, cursor: selectMode ? 'pointer' : 'default',
    boxShadow: checked ? `0 0 0 3px ${C.indigoSoft}` : 'none', transition:'all .12s',
  }}>
    {selectMode && <WCheckBox checked={checked}/>}
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ fontSize: 11, color: C.muted, marginTop: 2, fontWeight: 600 }}>{h.type}</div>
      <span style={{ display:'inline-flex', alignItems:'center', gap: 5, marginTop: 7, padding:'3px 8px', borderRadius: 999, background: C.amberSoft, color: C.amber, fontSize: 10, fontWeight: 700 }}>
        <Dot color={C.amber}/>Зарах захиалгатай
      </span>
    </div>
    <div style={{ textAlign:'right', flexShrink: 0 }}>
      <div style={{ fontSize: 13.5, fontWeight: 800, color: C.ink, fontVariantNumeric:'tabular-nums', letterSpacing:'-0.01em' }}>₮{h.val}</div>
      <div style={{ fontSize: 10.5, color: C.muted, marginTop: 3, fontWeight: 600, fontVariantNumeric:'tabular-nums' }}>Зарласан {h.listDate}</div>
    </div>
  </div>
);

const WalletBarChart = () => {
  // Monthly cash flow. Returns/income land every month (green, up);
  // loan repayments fall only on their due months (red, down) — sparse by design.
  const data = [
    { m:'01.26', ret: 1200000, loan: 0 },
    { m:'02.26', ret: 900000,  loan: 1100000 },
    { m:'03.26', ret: 1500000, loan: 0 },
    { m:'04.26', ret: 1100000, loan: 0 },
    { m:'05.26', ret: 1800000, loan: 950000 },
    { m:'06.26', ret: 1145000, loan: 0 },
  ];
  const W = 320, Hc = 134, zero = 64, half = 52, niceMax = 2000000;
  const sc = (v) => (v / niceMax) * half;
  const slot = W / data.length;
  const barW = 20;
  return (
    <div style={{ marginTop: 14, background:'#fff', borderRadius: 20, border:`1px solid ${C.line2}`, padding: 18 }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap: 8 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 800, color: C.ink }}>Сарын урсгал</div>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 600, marginTop: 2 }}>Сүүлийн 6 сар · ₮</div>
        </div>
        <div style={{ display:'flex', gap: 12 }}>
          <span style={{ display:'flex', alignItems:'center', gap: 5, fontSize: 11, fontWeight: 600, color: C.text }}><span style={{ width:9, height:9, borderRadius:3, background: C.green }}/>Буцаан олголт</span>
          <span style={{ display:'flex', alignItems:'center', gap: 5, fontSize: 11, fontWeight: 600, color: C.text }}><span style={{ width:9, height:9, borderRadius:3, background: C.red }}/>Зээлийн төлөлт</span>
        </div>
      </div>
      <div style={{ display:'flex', gap: 8, marginTop: 14 }}>
        {/* Y axis — amount (₮) */}
        <div style={{ display:'flex', flexDirection:'column', justifyContent:'space-between', height: Hc, fontSize: 9, color: C.muted2, fontWeight: 600, textAlign:'right', fontVariantNumeric:'tabular-nums' }}>
          <span>+₮2сая</span><span>0</span><span>−₮2сая</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <svg width="100%" viewBox={`0 0 ${W} ${Hc}`} preserveAspectRatio="none" style={{ display:'block', height: Hc }}>
            <line x1="0" y1={zero-half} x2={W} y2={zero-half} stroke={C.line2} strokeWidth="1" strokeDasharray="3 3"/>
            <line x1="0" y1={zero+half} x2={W} y2={zero+half} stroke={C.line2} strokeWidth="1" strokeDasharray="3 3"/>
            <line x1="0" y1={zero} x2={W} y2={zero} stroke={C.line} strokeWidth="1.5"/>
            {data.map((d,i)=>{
              const cx = i*slot + slot/2;
              const inH = sc(d.ret), outH = sc(d.loan);
              return (
                <g key={i}>
                  <rect x={cx-barW/2} y={zero-inH} width={barW} height={inH} rx="3" fill={C.green}/>
                  {d.loan > 0 && <rect x={cx-barW/2} y={zero} width={barW} height={outH} rx="3" fill={C.red}/>}
                </g>
              );
            })}
          </svg>
          ...
          <div style={{ display:'flex', marginTop: 4 }}>
            {data.map((d,i)=>(
              <div key={i} style={{ flex:1, textAlign:'center', fontSize: 9.5, color: C.muted, fontWeight: 600, fontVariantNumeric:'tabular-nums' }}>{d.m}</div>
            ))}
          </div>
        </div>
      </div>
...
const Wallet = ({ activeTab='wallet', onNav, view='default', listingView=null }) => {
  const cash = 12000000;
  const base = [
    { issuer:'Капитрон Банк', ticker:'CAPIT 1450', type:'Сертификат', ab:'CD', c: C.blue, valN: 8200000, val:'8,200,000', y:'15.5', mat:'278 хоног', matDays: 278, status:'' },
    { issuer:'Голден Хилл Партнерс', ticker:'GOLDH 2300', type:'Итгэлцэл', ab:'ИТ', c:'#F59E0B', valN: 12000000, val:'12,000,000', y:'25.6', mat:'327 хоног', matDays: 327, status:'onsale' },
    { issuer:'Кредитекс СТМ ББСБ', ticker:'MSTRT 2400', type:'Итгэлцэл', ab:'ИТ', c: C.indigo, valN: 6500000, val:'6,500,000', y:'21.3', mat:'5 хоног', matDays: 5, status:'soon' },
    { issuer:'Мерит Финанс ББСБ', ticker:'INV 0820', type:'Нэхэмжлэх', ab:'НЭ', c: C.green, valN: 4300000, val:'4,300,000', y:'18.0', mat:'45 хоног', matDays: 45, status:'' },
    { issuer:'Эм Си Эс Холдинг', ticker:'NEXT 7.5', type:'Арилжааны бичиг', ab:'АБ', c: C.orange, valN: 3800000, val:'3,800,000', y:'7.5', mat:'12 хоног', matDays: 12, status:'' },
  ];
  const extra = [
    { issuer:'Ди Эм Финанс ББСБ', ticker:'DMFIN 2250', type:'Итгэлцэл', ab:'ИТ', c:'#7C3AED', valN: 5200000, val:'5,200,000', y:'22.9', mat:'190 хоног', matDays: 190, status:'' },
    { issuer:'Зээли Капитал ББСБ', ticker:'ZEELY 2100', type:'Итгэлцэл', ab:'ИТ', c:'#0EA5A5', valN: 2100000, val:'2,100,000', y:'21.4', mat:'141 хоног', matDays: 141, status:'onsale' },
    { issuer:'Капитрон Банк', ticker:'CAPIT 1620', type:'Сертификат', ab:'CD', c: C.blue, valN: 9400000, val:'9,400,000', y:'14.8', mat:'310 хоног', matDays: 310, status:'' },
  ];
  ...
  const listDates = { 'GOLDH 2300':'2026-05-18', 'ZEELY 2100':'2026-05-20' };
  const listings = holdings.filter(h => h.status === 'onsale').map(h => ({ ...h, listDate: listDates[h.ticker] || '2026-05-19' }));
  const initSel = listingView === 'confirm' ? listings.map(l => l.ticker) : (listingView === 'select' && listings[0] ? [listings[0].ticker] : []);
  const [selMode, setSelMode] = useState(!!listingView);
  const [sel, setSel] = useState(initSel);
  const [confirmOpen, setConfirmOpen] = useState(listingView === 'confirm');
  const [pinOpen, setPinOpen] = useState(false);
  const [toast, setToast] = useState(false);
  const [holdFilter, setHoldFilter] = useState('Бүгд');
  const holdTypes = ['Бүгд', ...Array.from(new Set(holdings.map(h => h.type)))];
  const shownHoldings = holdFilter === 'Бүгд' ? holdings : holdings.filter(h => h.type === holdFilter);
  const toggleSel = (tk) => setSel(s => s.includes(tk) ? s.filter(x => x !== tk) : [...s, tk]);
  const allSelected = listings.length > 0 && sel.length === listings.length;
  const toggleAll = () => setSel(allSelected ? [] : listings.map(l => l.ticker));
  const exitSel = () => { setSelMode(false); setSel([]); };
  const doCancel = () => { setPinOpen(false); setConfirmOpen(false); exitSel(); setToast(true); setTimeout(()=>setToast(false), 2600); };

  // ── Pure empty state — nothing funded yet ──
  if (view === 'empty') {
    return (
      <Frame label="28A — Wallet · хоосон">
        ...
        <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
          ...
          <div style={{ borderRadius: 22, padding: 20, background:`linear-gradient(135deg, ${C.navy} 0%, ${C.navy3} 100%)`, color:'#fff', position:'relative', overflow:'hidden' }}>
            <div style={{ position:'absolute', right:-40, top:-40, opacity:.25 }}><LogoMark size={140}/></div>
            <div style={{ position:'relative' }}>
              ...
              <div style={{ fontSize: 36, fontWeight: 800, marginTop: 10, letterSpacing:'-0.02em', fontVariantNumeric:'tabular-nums' }}>₮ 0</div>
              ...
              <div style={{ display:'flex', gap: 8, marginTop: 18 }}>
                ...
                <div style={{ flex: 1, height: 44, borderRadius: 12, background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.12)', color:'rgba(255,255,255,.4)', fontWeight: 700, fontSize: 13, display:'flex', alignItems:'center', justifyContent:'center', gap: 6 }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M12 19V5M5 12l7 7 7-7" stroke="rgba(255,255,255,.4)" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  Зарлага
                </div>
              </div>
            </div>
        ...
        {/* LINKED CARDS — entry to card manager */}
        <button onClick={()=>onNav && onNav('myCards')} style={{ width:'100%', marginTop: 14, background:'#fff', borderRadius: 18, border:`1px solid ${C.line2}`, padding:'14px 16px', display:'flex', alignItems:'center', gap: 13, cursor:'pointer', textAlign:'left' }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: C.indigoSoft, display:'flex', alignItems:'center', justifyContent:'center', flexShrink: 0 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><rect x="3" y="6" width="18" height="12" rx="3" stroke={C.indigo} strokeWidth="2"/><path d="M3 10h18" stroke={C.indigo} strokeWidth="2"/><circle cx="16.5" cy="14.5" r="1.3" fill={C.indigo}/></svg>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: C.ink }}>Миний картууд</div>
            <div style={{ fontSize: 11.5, color: C.muted, marginTop: 2 }}>Цэнэглэлт, автомат төлөлтөд ашиглах картууд</div>
          </div>
          <span style={{ fontSize: 12, fontWeight: 700, color: C.muted, flexShrink: 0 }}>3 карт</span>
          ...
          {/* empty-state card */}
          <div style={{ marginTop: 14, background:'#fff', borderRadius: 20, border:`1px dashed ${C.line}`, padding:'36px 24px', textAlign:'center' }}>
            <div style={{ width: 66, height: 66, borderRadius: 19, background: C.indigoSoft, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto' }}>
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><rect x="3" y="7" width="18" height="12" rx="3" stroke={C.indigo} strokeWidth="2"/><path d="M3 11h18" stroke={C.indigo} strokeWidth="2"/><circle cx="16.5" cy="15" r="1.4" fill={C.indigo}/></svg>
            </div>
            <div style={{ fontSize: 16, fontWeight: 800, color: C.ink, marginTop: 16, letterSpacing:'-0.01em' }}>Таны багц хоосон байна</div>
            <div style={{ fontSize: 12.5, color: C.muted, marginTop: 8, lineHeight: 1.55, maxWidth: 264, marginLeft:'auto', marginRight:'auto' }}>Хэтэвчээ цэнэглээд эхний бүтээгдэхүүнээ авснаар багц, өгөөж, гүйлгээний түүх энд харагдана.</div>
            <button onClick={()=>onNav && onNav('addAmount')} style={{ width:'100%', height: 48, borderRadius: 14, background: C.indigo, color:'#fff', border:'none', fontWeight: 700, fontSize: 14, cursor:'pointer', marginTop: 20, boxShadow:'0 8px 22px -8px rgba(79,70,229,.5)' }}>Хэтэвч цэнэглэх</button>
            <button onClick={()=>onNav && onNav('trade')} style={{ width:'100%', height: 46, borderRadius: 14, background:'#fff', color: C.ink, border:`1.5px solid ${C.line}`, fontWeight: 700, fontSize: 14, cursor:'pointer', marginTop: 10 }}>Бүтээгдэхүүн үзэх</button>
          </div>
        </div>
        <BottomTabs active={activeTab} onNav={onNav}/>
      </Frame>
  ...
  return (
    <Frame label="28 — Wallet · Багц">
      ...
      <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
        ...
        {window.AutoInvestWalletSection && React.createElement(window.AutoInvestWalletSection, { onNav })}
        <WalletBarChart/>
        ...
        {holdings.length > 0 && holdTypes.length > 2 && (
          <div style={{ display:'flex', gap: 8, marginTop: 12, overflowX:'auto', marginLeft:-24, marginRight:-24, paddingLeft:24, paddingRight:24 }}>
            {holdTypes.map((t, i) => (
              <div key={i} onClick={()=>setHoldFilter(t)} data-nodrag style={{
                padding:'7px 13px', borderRadius:999, whiteSpace:'nowrap',
                background: holdFilter===t ? C.ink : '#fff', color: holdFilter===t ? '#fff' : C.text,
                border:`1px solid ${holdFilter===t ? C.ink : C.line}`,
        ...
        {holdings.length === 0 ? (
          ...
          <div style={{ marginTop: 12, display:'flex', flexDirection:'column', gap: 10 }}>
            {shownHoldings.map((h, i) => <WHolding key={i} h={h} onNav={onNav}/>)}
          </div>
        )}

        {/* SECONDARY-MARKET LISTINGS section */}
        <div style={{ marginTop: 22, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ fontSize: 14, fontWeight: 800, color: C.ink, letterSpacing:'-0.01em' }}>Зарагдаж байгаа{listings.length ? ' (' + listings.length + ')' : ''}</div>
          {listings.length > 0 && (
            selMode
              ? <button onClick={exitSel} style={{ fontSize: 12.5, fontWeight: 700, color: C.muted, background:'none', border:'none', cursor:'pointer' }}>Болих</button>
              : <button onClick={()=>setSelMode(true)} style={{ fontSize: 12.5, fontWeight: 700, color: C.indigo, background:'none', border:'none', cursor:'pointer' }}>Сонгох</button>
          )}
        </div>

        {listings.length === 0 ? (
          <div style={{ marginTop: 12, background:'#fff', borderRadius: 16, border:`1px dashed ${C.line}`, padding:'20px', display:'flex', alignItems:'center', gap: 14 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background:'#F4F5F9', display:'flex', alignItems:'center', justifyContent:'center', flexShrink: 0 }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3.5 7.5h17l-1.4 12.2a1 1 0 01-1 .8H5.9a1 1 0 01-1-.8L3.5 7.5z" stroke={C.muted2} strokeWidth="2" strokeLinejoin="round"/><path d="M8.5 7.5V6a3.5 3.5 0 017 0v1.5" stroke={C.muted2} strokeWidth="2"/></svg>
            </div>
            <div style={{ fontSize: 12.5, color: C.muted, fontWeight: 600, lineHeight: 1.5 }}>Одоогоор зарах захиалга байхгүй байна</div>
          </div>
        ) : (
          <React.Fragment>
            {selMode && (
              <button onClick={toggleAll} style={{ width:'100%', marginTop: 12, display:'flex', alignItems:'center', gap: 12, padding:'11px 14px', background:'#fff', border:`1px solid ${C.line2}`, borderRadius: 12, cursor:'pointer' }}>
                <WCheckBox checked={allSelected}/>
                <span style={{ fontSize: 12.5, fontWeight: 700, color: C.ink }}>Бүгдийг сонгох</span>
                <span style={{ marginLeft:'auto', fontSize: 11.5, color: C.muted, fontWeight: 600 }}>{sel.length} / {listings.length} сонгогдсон</span>
              </button>
            )}
            <div style={{ marginTop: 12, display:'flex', flexDirection:'column', gap: 10 }}>
              {listings.map((h, i) => (
                <WListing key={i} h={h} selectMode={selMode} checked={sel.includes(h.ticker)} onToggle={()=>toggleSel(h.ticker)}/>
              ))}
            </div>
          </React.Fragment>
        )}
        ...
        <div style={{ marginTop: 18, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <span onClick={()=>onNav && onNav('txHistory')} style={{ fontSize: 12, color: C.indigo, fontWeight: 700, cursor:'pointer' }}>Бүгд →</span>
        </div>
      ...
      {selMode ? (
        <div style={{ padding:'12px 24px 14px', background:'#fff', borderTop:`1px solid ${C.line2}`, flexShrink: 0 }}>
          <button disabled={sel.length === 0} onClick={()=>setConfirmOpen(true)} style={{
            width:'100%', height: 50, borderRadius: 14, border:'none',
            background: sel.length === 0 ? '#E7E9F2' : C.red,
            color: sel.length === 0 ? C.muted2 : '#fff', fontWeight: 700, fontSize: 15,
            cursor: sel.length === 0 ? 'default' : 'pointer',
            display:'flex', alignItems:'center', justifyContent:'center', gap: 8,
          }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M7 7l10 10M17 7L7 17" stroke={sel.length === 0 ? C.muted2 : '#fff'} strokeWidth="2.4" strokeLinecap="round"/></svg>
            Захиалга цуцлах{sel.length ? ' (' + sel.length + ')' : ''}
          </button>
        </div>
      ) : (
        <BottomTabs active={activeTab} onNav={onNav}/>
      ...
      {confirmOpen && !pinOpen && (
        <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,.5)', display:'flex', flexDirection:'column', justifyContent:'flex-end', zIndex:20 }}>
          <div style={{ background:'#fff', borderRadius:'22px 22px 0 0', padding:'24px 24px 30px' }}>
            <div style={{ width:40, height:4, borderRadius:999, background:C.line, margin:'0 auto 20px' }}/>
            <div style={{ width: 52, height: 52, borderRadius: 16, background: C.amberSoft, display:'flex', alignItems:'center', justifyContent:'center', marginBottom: 16 }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none"><path d="M12 8v5M12 16h.01" stroke={C.amber} strokeWidth="2.4" strokeLinecap="round"/><circle cx="12" cy="12" r="9" stroke={C.amber} strokeWidth="2"/></svg>
            </div>
            <div style={{ fontSize:18, fontWeight:800, color:C.ink, letterSpacing:'-0.01em', lineHeight:1.3 }}>
              {sel.length} захиалга цуцлах уу?
            </div>
            <div style={{ fontSize:13, color:C.muted, marginTop:10, lineHeight:1.6 }}>
              Сонгосон зарах захиалга цуцлагдаж, хоёрдогч зах зээлээс хасагдана. Дахин зарахын тулд шинээр захиалга үүсгэнэ.
            </div>
            <div style={{ marginTop:20, display:'flex', flexDirection:'column', gap:10 }}>
              <button onClick={()=>setConfirmOpen(false)} style={{ width:'100%', height:52, borderRadius:14, background:C.indigo, color:'#fff', border:'none', fontWeight:700, fontSize:15, cursor:'pointer', boxShadow:'0 8px 22px -8px rgba(79,70,229,.5)' }}>Болих</button>
              <button onClick={()=>setPinOpen(true)} style={{ width:'100%', height:48, borderRadius:14, background:'transparent', color:C.red, border:`1.5px solid ${C.red}`, fontWeight:700, fontSize:14, cursor:'pointer' }}>Захиалга цуцлах</button>
            </div>
          </div>
        </div>
      )}
      ...
      {pinOpen && (
        <div style={{ position:'absolute', inset:0, background:'#fff', display:'flex', flexDirection:'column', zIndex:30 }}>
          <div style={{ height: 52, display:'flex', alignItems:'center', padding:'0 12px', flexShrink: 0 }}>
            <button onClick={()=>setPinOpen(false)} style={{ width:40, height:40, borderRadius:12, background:'#fff', border:`1px solid ${C.line}`, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M15 6l-6 6 6 6" stroke={C.ink} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
            <div style={{ flex:1, textAlign:'center', fontSize:15, fontWeight:700, color:C.ink, marginRight: 40 }}>Гүйлгээний ПИН код</div>
          </div>
          <div style={{ flex:1, overflow:'auto', padding:'14px 24px 20px', display:'flex', flexDirection:'column' }}>
            <div style={{ fontSize:21, fontWeight:800, color:C.ink, letterSpacing:'-0.02em' }}>ПИН кодоо оруулна уу</div>
            <div style={{ fontSize:12.5, color:C.muted, marginTop:8, lineHeight:1.5 }}>{sel.length} зарах захиалгыг цуцлахыг баталгаажуулна уу.</div>
            <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', paddingTop:16 }}>
              <TransactionPin onFilled={doCancel}/>
            </div>
          </div>
        </div>
      )}
      ...
      {toast && (
        <div style={{ position:'absolute', left: 0, right: 0, bottom: 96, display:'flex', justifyContent:'center', zIndex:40, pointerEvents:'none' }}>
          <div style={{ display:'inline-flex', alignItems:'center', gap: 8, padding:'12px 18px', borderRadius: 14, background: C.ink, color:'#fff', fontSize: 13, fontWeight: 700, boxShadow:'0 12px 30px -8px rgba(0,0,0,.4)' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M5 12l4 4 10-10" stroke="#7FF3C2" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
            Зарах захиалга цуцлагдлаа
          </div>
        </div>
      )}
    </Frame>
...
// ============================================================
const EduDetail = ({ onNav }) => (
  <NewsDetail onNav={onNav} article={{
    tag:'Суралцах',
    date:'4 мин унших',
    title:'Мөнгөний зах зээлийн сан гэж юу вэ?',
    lead:'Мөнгөний зах зээлийн сан (Money Market Fund) нь богино хугацаат, бага эрсдэлтэй санхүүгийн хэрэгслүүдэд хөрөнгө оруулдаг нэгдсэн сан юм.',
    body:[
      'Сан нь олон хөрөнгө оруулагчийн мөнгийг нэгтгэн, төрийн болон компанийн богино хугацаат бичиг, хадгаламжийн сертификат зэрэгт байршуулдаг. Ингэснээр ганц хүний хувьд хүрэхэд хэцүү боломжуудад хамтдаа оролцоно.',
      'Гол давуу тал нь хөрвөх чадвар буюу мөнгөө харьцангуй хурдан буцаан авах боломж, тогтвортой өгөөж юм. Гэхдээ өгөөж нь баталгаатай биш, зах зээлийн нөхцөлөөс хамаарч өөрчлөгдөж болохыг анхаарна уу.',
    ],
    facts:[
      { l:'Түвшин', v:'Шинэхэн' },
      { l:'Унших хугацаа', v:'4 минут' },
      { l:'Сэдэв', v:'Үндсэн ойлголт' },
    ],
  }}/>
);
...
const Loan = ({ activeTab='loan', onNav, multi=false }) => {
  const daysLeft = 24;
  const L_PRINCIPAL = 3000000;          // анхны зээл
  const L_PAID      = 1200000;          // өнөөг хүртэл төлсөн
  const L_REMAIN    = L_PRINCIPAL - L_PAID;     // үлдэгдэл үндсэн
  const L_ACCRUED   = 15000;            // хуримтлагдсан хүү
  const L_PAYOFF    = L_REMAIN + L_ACCRUED;     // төлж хаах үлдэгдэл
  const repaidPct   = Math.round((L_PAID / L_PRINCIPAL) * 100);
  const lf = (n) => n.toLocaleString('en-US');
  const [showList, setShowList] = useState(multi);
  const loanCards = [
    { id:'LN-2026-04823', payoff:'1,815,000', principal:'3,000,000', paid:'1,200,000', repaidPct: 40, daysLeft:24, due:'2026-06-28', status:'Хэсэгчлэн төлсөн', tone:'info' },
    { id:'LN-2026-05110', payoff:'1,510,000', principal:'1,500,000', paid:null,        repaidPct: 0,  daysLeft:8,  due:'2026-07-12', status:'Хэвийн', tone:'new' },
  ];
  const bulkTotal = '3,325,000';
  if (showList) {
    return (
      <Frame label="30L — Loan list">
        ...
        <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
          ...
          {/* Bulk close — subtle, not a competing primary */}
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap: 12, padding:'12px 14px', marginBottom: 12, background:'#FAFBFE', border:`1px dashed ${C.line}`, borderRadius: 14 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: C.ink, letterSpacing:'-0.01em' }}>Бүх зээлээ нэг дор хаах</div>
              <div style={{ fontSize: 11, color: C.muted, marginTop: 2, fontVariantNumeric:'tabular-nums' }}>Нийт төлж хаах ₮ {bulkTotal}</div>
            </div>
            <div role="button" tabIndex={0} data-nodrag onClick={(e) => { e.stopPropagation(); onNav && onNav('payoffConfirm'); }} style={{
              flexShrink: 0, display:'inline-flex', alignItems:'center', gap: 6, padding:'9px 14px', borderRadius: 11, cursor:'pointer',
              background:'#fff', border:`1.5px solid ${C.indigo}`, color: C.indigo, fontSize: 12.5, fontWeight: 700, letterSpacing:'-0.01em',
            }}>
              Бүгдийг хаах
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M9 6l6 6-6 6" stroke={C.indigo} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap: 12 }}>
            {loanCards.map((L, i) => (
              <button key={i} onClick={() => setShowList(false)} style={{
                ...
                <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginTop: 14 }}>
                  <div>
                    <div style={{ fontSize: 10.5, color: C.muted, fontWeight: 600 }}>Төлж хаах үлдэгдэл</div>
                ...
                <div style={{ marginTop: 12, height: 6, borderRadius: 999, background: C.line2, overflow:'hidden' }}>
                  <div style={{ width: Math.max(L.repaidPct, 2) + '%', height:'100%', borderRadius: 999, background:`linear-gradient(90deg, ${C.green}, ${C.blue})` }}/>
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', marginTop: 8, fontSize: 10.5, color: C.muted, fontWeight: 600, fontVariantNumeric:'tabular-nums' }}>
                  <span>{L.paid ? `Төлсөн ₮ ${L.paid}` : 'Төлбөр эхлээгүй'}</span>
                  <span>Анхны ₮ {L.principal}</span>
                </div>
                <div style={{ display:'flex', gap: 8, marginTop: 14 }}>
                  <div role="button" tabIndex={0} data-nodrag onClick={(e) => { e.stopPropagation(); onNav && onNav('partialPay'); }} style={{
                    flex: 1, textAlign:'center', padding:'10px 12px', borderRadius: 12, cursor:'pointer',
                    background:'#fff', border:`1px solid ${C.line}`, color: C.ink, fontSize: 12.5, fontWeight: 700, letterSpacing:'-0.01em',
                  }}>Хувааж төлөх</div>
                  <div role="button" tabIndex={0} data-nodrag onClick={(e) => { e.stopPropagation(); onNav && onNav('payoffConfirm'); }} style={{
                    flex: 1, textAlign:'center', padding:'10px 12px', borderRadius: 12, cursor:'pointer',
                    background: C.indigo, border:`1px solid ${C.indigo}`, color:'#fff', fontSize: 12.5, fontWeight: 700, letterSpacing:'-0.01em',
                  }}>Зээл хаах</div>
                </div>
              </button>
            ))}
      ...
      <div style={{ flex: 1, overflow:'auto', padding: '0 24px 100px' }}>
        ...
        <div style={{
          ...
          <div style={{ position:'relative' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              <Badge tone="info">Хэсэгчлэн төлсөн</Badge>
            </div>

            {/* P1 — pending to close */}
            <div style={{ fontSize: 12, opacity: .7, marginTop: 16 }}>Төлж хаахад үлдсэн</div>
            <div style={{ fontSize: 34, fontWeight: 800, letterSpacing:'-0.02em', fontVariantNumeric:'tabular-nums', marginTop: 2 }}>
              ₮ {lf(L_PAYOFF)}
            </div>
            {/* subtle — original loan amount */}
            <div style={{ fontSize: 11, opacity: .55, fontWeight: 600, marginTop: 6, fontVariantNumeric:'tabular-nums' }}>
              Анхны зээл ₮ {lf(L_PRINCIPAL)}
            </div>

            {/* repayment progress */}
            <div style={{ marginTop: 16 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize: 11, opacity:.7, marginBottom: 6 }}>
                <span style={{ fontVariantNumeric:'tabular-nums' }}>{repaidPct}% төлөгдсөн</span>
              ...
              <div style={{ height: 6, borderRadius: 999, background:'rgba(255,255,255,.15)', overflow:'hidden' }}>
                <div style={{ width: repaidPct+'%', height:'100%', borderRadius: 999, background:'linear-gradient(90deg, #7FF3C2, #2D6BFF)' }}/>
              </div>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap: 10, marginTop: 18 }}>
              ...
              <div style={{ background:'rgba(255,255,255,.06)', backdropFilter:'blur(10px)', border:'1px solid rgba(255,255,255,.1)', borderRadius: 12, padding: 12 }}>
                <div style={{ fontSize: 10, opacity: .6, fontWeight: 600 }}>Үлдэгдэл үндсэн</div>
                <div style={{ fontSize: 14, fontWeight: 700, marginTop: 4, fontVariantNumeric:'tabular-nums' }}>₮ {lf(L_REMAIN)}</div>
                <div style={{ fontSize: 10, opacity: .65, marginTop: 2 }}>Хүү 2.5% / 30 хоног</div>
              </div>
            </div>
          </div>
        </div>

        {/* Two actions: partial payment · close loan */}
        <div style={{ display:'flex', gap: 10, marginTop: 14 }}>
          <button onClick={() => onNav && onNav('partialPay')} style={{ flex: 1, height: 52, borderRadius: 14, background:'#fff', color: C.ink, border:`1.5px solid ${C.line}`, fontWeight: 700, fontSize: 15, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap: 8 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M4 12h4M10 12h4M16 12h4" stroke={C.ink} strokeWidth="2.4" strokeLinecap="round"/></svg>
            Хувааж төлөх
          </button>
          <button onClick={() => onNav && onNav('payoffConfirm')} style={{ flex: 1, height: 52, borderRadius: 14, background: C.indigo, color:'#fff', border:'none', fontWeight: 700, fontSize: 15, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap: 8, boxShadow:'0 8px 22px -8px rgba(79,70,229,.5)' }}>
        ...
        <div style={{ marginTop: 10, background:'#fff', borderRadius: 18, border:`1px solid ${C.line2}`, overflow:'hidden' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 16px', borderBottom:`1px solid ${C.line2}` }}>
            <span style={{ fontSize: 12.5, color: C.muted, fontWeight: 600 }}>Анхны зээл</span>
            <span style={{ fontSize: 13.5, color: C.ink, fontWeight: 700, fontVariantNumeric:'tabular-nums' }}>₮ {lf(L_PRINCIPAL)}</span>
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 16px', borderBottom:`1px solid ${C.line2}` }}>
            <span style={{ fontSize: 12.5, color: C.muted, fontWeight: 600 }}>Төлсөн дүн</span>
            <span style={{ fontSize: 13.5, color: C.green, fontWeight: 700, fontVariantNumeric:'tabular-nums' }}>− ₮ {lf(L_PAID)}</span>
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 16px', borderBottom:`1px solid ${C.line2}` }}>
            <span style={{ fontSize: 12.5, color: C.muted, fontWeight: 600 }}>Үлдэгдэл үндсэн</span>
            <span style={{ fontSize: 13.5, color: C.ink, fontWeight: 700, fontVariantNumeric:'tabular-nums' }}>₮ {lf(L_REMAIN)}</span>
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'13px 16px', borderBottom:`1px solid ${C.line2}` }}>
            <span style={{ fontSize: 12.5, color: C.muted, fontWeight: 600 }}>Хуримтлагдсан хүү</span>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'15px 16px', background:'#FAFBFE' }}>
            <span style={{ fontSize: 13, color: C.ink, fontWeight: 800 }}>Төлж хаах үлдэгдэл</span>
        ...
        <div style={{ marginTop: 12, display:'flex', gap: 10, alignItems:'flex-start', padding: 13, borderRadius: 12, background: C.indigoSoft }}>
          ...
          <div style={{ fontSize: 12, color: C.ink, lineHeight: 1.5 }}>
            <strong>Хувааж төлөх</strong> — үлдэгдлийн дурын хэсгийг төлж, хүүгээ бууруулна. <strong>Зээл хаах</strong> — үлдэгдлийг бүрэн төлж зээлээ хаана.
          </div>
        </div>
        ...
        <div style={{ marginTop: 10, background:'#fff', borderRadius: 18, border:`1px solid ${C.line2}`, overflow:'hidden' }}>
          {[
            { t:'Зээл хэсэгчлэн төлсөн', d:'2026.06.10 · 09:32', sub:'Хэтэвчээс суутгасан', v:'− ₮ 1,200,000', tone: C.indigo, soft: C.indigoSoft, up:false },
    ...
    <Frame label="33 — Loan eligibility · KYC">
      ...
      <div style={{ flex: 1, overflow:'auto', padding:'6px 24px 20px' }}>
        ...
        <div style={{ marginTop: 22, display:'flex', flexDirection:'column', gap: 18 }}>
          ...
          <div data-nodrag onClick={() => setConsent(!consent)} style={{
...
Object.assign(window, {
  ...
  Home, Trading, Category, Detail, Order, Wallet, News, NewsDetail, EduDetail, Loan,
  PrimaryMarket,
